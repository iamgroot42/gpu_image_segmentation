...
__global__ void push(Pixel *image_graph, unsigned long long *F, Terminal *source, Terminal *sink, int height, int width, int *convergence_flag){
	...
	if (i <= height && j <= width){
		...
		int flowToPush = shared_excess[locali + 1][localj + 1];
		...
		for(int l = 0; l < 8; l++){
			...
			if(shared_heights[dest_x][dest_y] + 1 == shared_heights[locali + 1][localj + 1]){
				...
				if (neighbor_flows[l] < neighbor_capacities[l])
				{
					if (flowToPush > neighbor_capacities[l] - neighbor_flows[l])
						flowToPush = neighbor_capacities[l] - neighbor_flows[l];
					shared_excess[dest_x][dest_y] += flowToPush;
					shared_excess[locali + 1][localj + 1] -= flowToPush;
					neighbor_flows[l] += flowToPush;
					image_graph[(dest_x + 1) * width + (dest_y + 1)].neighbor_flows[7 - l] -= flowToPush;
				}
				thread_flag = 1;
			}
		}
